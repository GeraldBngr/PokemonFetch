...
const savePokemonSprites = async (folderName, pokemonSpritesObject) => {
  for (const [key, url] of Object.entries(pokemonSpritesObject)) {
    ...
    if (typeof url === "string") { //if urlis a string(link for the picture)
...
savePokemonStats("flareon", pokemonObject.stats);
savePokemonArtwork("flareon", pokemonObject.sprites);
savePokemonSprites("flareon", pokemonObject.sprites);
